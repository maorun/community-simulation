...
- **Wealth Inequality Analysis:** Comprehensive wealth distribution analysis including:
  ...
  - **Lorenz Curve Data:** Visualization-ready coordinate pairs showing the cumulative distribution of wealth across the population. Each point represents (cumulative % of population, cumulative % of wealth), starting at (0.0, 0.0) and ending at (1.0, 1.0). The curve enables graphical analysis of inequality - the diagonal line y=x represents perfect equality, and curves below the diagonal indicate inequality (with greater distance showing more extreme inequality). The area between the Lorenz curve and the perfect equality line is directly related to the Gini coefficient. This data is automatically calculated and included in JSON output under `money_statistics.lorenz_curve` for use with visualization tools like matplotlib, gnuplot, or web-based charting libraries. Perfect for creating publication-quality inequality visualizations and comparing wealth distributions across different simulation scenarios.
