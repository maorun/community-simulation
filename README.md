...
- **Tax System:** Government entity that collects taxes from persons' income and optionally redistributes them equally. Simulates wealth redistribution effects and fiscal policy impacts on the economy. Taxes are collected as a percentage of seller proceeds after transaction fees. Enable via `--enable-taxes` flag with configurable `--tax-rate` (0.0-1.0 range, e.g., 0.1 for 10% tax rate). Tax redistribution can be disabled with `--no-tax-redistribution` flag to simulate government spending without direct transfers. Tax statistics (total collected, total redistributed, averages per person and per step) are tracked and reported in results.
