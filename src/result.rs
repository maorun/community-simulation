...
/// Statistics about asset ownership and wealth accumulation through assets
#[derive(Debug, Serialize, Deserialize, Clone)]
pub struct AssetStats {
    /// Total number of assets purchased during the simulation
    pub total_assets_purchased: usize,
    /// Number of active assets at simulation end
    pub active_assets: usize,
    /// Total value of all assets at simulation end
    pub total_asset_value: f64,
    /// Average asset value
    pub avg_asset_value: f64,
    /// Median asset value
    pub median_asset_value: f64,
    /// Minimum asset value
    pub min_asset_value: f64,
    /// Maximum asset value
    pub max_asset_value: f64,
    /// Total income generated by all assets (rental income, dividends, etc.)
    pub total_income_generated: f64,
    /// Average ROI (return on investment) across all assets
    pub avg_roi: f64,
    /// Number of assets by type (Property, Equipment, Stocks)
    pub assets_by_type: HashMap<String, usize>,
    /// Total value by asset type
    pub value_by_type: HashMap<String, f64>,
    /// Average number of assets per person
    pub avg_assets_per_person: f64,
    /// Percentage of persons who own at least one asset
    pub asset_ownership_rate: f64,
}

impl AssetStats {
    /// Creates a new AssetStats with default values (no assets).
    pub fn new() -> Self {
        Self {
            ...
            avg_assets_per_person: 0.0,
            asset_ownership_rate: 0.0,
        }
    }
}
...
pub struct SimulationResult {
    ...
    /// Asset ownership statistics (only present if assets are enabled)
    #[serde(skip_serializing_if = "Option::is_none")]
    pub asset_statistics: Option<AssetStats>,
    ...
    let per_person: Vec<PersonTradingStats> = if active_entities.len() > 100 {
        let mut stats: Vec<PersonTradingStats> = active_entities
            ...
            .map(|entity| {
                ...
                partner_list.sort_by_key(|item| Reverse(item.1));
