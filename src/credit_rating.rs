...
impl CreditScore {
    ...
    pub fn calculate_score(&mut self, current_debt: f64, current_money: f64, current_step: usize) {
        ...
        if self.credit_history_steps == 0 && self.successful_payments == 0 && self.missed_payments == 0 {
    ...
    fn calculate_payment_history_factor(&self) -> f64 {
        ...
        success_rate.powf(2.0).max(0.0).min(1.0)
    }
    ...
    fn calculate_debt_factor(&self, current_debt: f64, current_money: f64) -> f64 {
        ...
        (1.0 - debt_ratio * 1.5).max(0.1).min(1.0)
    }
...
mod tests {
    ...
    fn test_credit_history_length_improves_score() {
        ...
        assert!(long_history_score > short_history_score, "Longer credit history should yield higher score");
    }
