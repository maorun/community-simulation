...
Persons can purchase insurance policies to protect against economic risks. When enabled, persons randomly attempt to buy insurance each step based on a configurable probability (default: 5% per step). Three types of insurance are available:
  ...
  ```bash
  ./community-simulation run --enable-insurance \
...
### Demographic Dynamics and Aging Population

Simulate demographic transitions, agent lifecycle aging, productivity changes over life stages, retirement pensions, and generational inheritance/succession.
  - **Lifecycle & Productivity Curve:** Agents age with every simulation step. Productivity follows a lifecycle curve: rising during youth (< 20), peaking during working age (20 to retirement age), and gradually declining after retirement age.
  - **Retirement & Pension System:** Upon reaching `retirement_age` (default: 65), working-age agents contribute a percentage (`pension_contribution_rate`, default: 5%) of their income to a central pension pool. Pension funds are distributed equally to retired agents each step.
  - **Generational Succession & Inheritance:** When agents reach `max_age` (default: 80), they pass away. 80% of their accumulated wealth is inherited by a new entering successor agent (age 0) who receives their skills and capital, while 20% is collected as estate tax.

  Enable via configuration file with `enable_demographics: true` or by running with `--preset demographic_transition`. Configurable parameters: `default_retirement_age`, `default_max_age`, and `pension_contribution_rate`. Ideal for macroeconomic research on aging societies, pension sustainability, generational wealth transfer, and labor force dynamics.
